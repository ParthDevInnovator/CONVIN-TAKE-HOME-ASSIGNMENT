...
//
// This method is kept for existing store tests. Production ingestion uses
// ProcessEvent so that event insertion, call upsert, and stats update happen
// atomically.
...
// ProcessEvent atomically stores a new event, its call record, and the
// account statistics.
//
// It returns:
//   - true  when a new event was processed
//   - false when event_id was already processed
//
// The UNIQUE constraint on events.event_id provides the idempotency
// guarantee, while the transaction keeps all durable changes atomic.
func (s *Store) ProcessEvent(ctx context.Context, e Event) (bool, error) {
	...
	// The event already exists. Nothing else should be changed.
	if errors.Is(err, pgx.ErrNoRows) {
		return false, nil
	}
	...
	// Update the durable per-account aggregate.
	_, err = tx.Exec(
		ctx,
...
//
// This method is kept for existing store tests. Production ingestion uses
// ProcessEvent so this operation participates in the same transaction as the
// event and account stats updates.
...
// This method is kept for existing store tests. Production ingestion uses
// ProcessEvent so this operation participates in the same transaction as the
// event and call updates.
func (s *Store) IncrementAccountStats(
	ctx context.Context,
	accountID string,
	durationSec int,
) error {
...
func (s *Store) AccountStats(
	...
	err := s.pool.QueryRow(
		...
		`SELECT call_count, total_duration_sec
		 ...
		 WHERE account_id = $1`,
		accountID,
	).Scan(&st.CallCount, &st.TotalDurationSec)
